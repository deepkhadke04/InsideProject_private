...
function AddProducts() {
    ...
    const validate = () => {
        ...
        if (!selectedProduct) {
            newErrors.selectedProduct = 'Product selection is required';
        }
        if (!price || isNaN(price) || parseFloat(price) <= 0) {
            newErrors.price = 'Price is required and should be a positive number';
        }
        if (!quantity || isNaN(quantity) || parseInt(quantity) <= 0) {
            newErrors.quantity = 'Quantity is required and should be a positive integer';
        }
        if (!description) {
    ...
    const uploadImage = async (file, sellerProductId, imageField) => {
        ...
        formData.append('imageField', imageField); // Added to indicate which image field is being uploaded
    ...
    const handleSubmit = async (e) => {
        ...
        if (validate()) {
            try {
                ...
                const sellerId = localStorage.getItem('sellerId'); // Retrieve sellerId from local storage
                if (!sellerId) {
                    throw new Error('Seller ID not found in local storage');
                }
                ...
                if (!productResponse.ok) {
                    alert('Failed to save the seller product');
                    throw new Error('Failed to save the seller product');
                }
                ...
                if (image2) await uploadImage(image2, sellerProductId, 'image2');
                if (image3) await uploadImage(image3, sellerProductId, 'image3');
